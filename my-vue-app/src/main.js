...
class Drager {
  ...
  onMouseDown(e) {
    ...
    Drager.zIndex = Drager.zIndex > zIndex ? Number(Drager.zIndex) + 1 : Number(zIndex) + 1
    this.setEleStyle({ "zIndex": Drager.zIndex, position: 'fixed', 'cursor': 'move' })
...
app.directive('drager', {
  mounted(el, binding) {
    new Drager(el, binding.value || {})
  }
});
